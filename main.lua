  ...
  Game = 
  {
    ...
    Player = 
    {
      ...
      Update = function(self, dt)
        ...
        local MapX, MapY = math.floor(((self.X+20)%41)-20), math.floor(((self.Y+20)%41)-20)
        local Rc, Rl = math.floor((self.NextX+20)/41), math.floor((self.NextY+20)/41)
        local Rx, Ry = math.floor((self.X+20)/41), math.floor((self.Y+20)/41)
        if Ry >= Game.Maps.MinY and Ry <= Game.Maps.MaxY and Rc >= Game.Maps.MinX and Rc <= Game.Maps.MaxX then
          if not Game.Maps.Region[Ry][Rc].Grid[MapY][c].Entity or not Game.Maps.Region[Ry][Rc].Grid[MapY][c].Entity.Collide then
            self.X = self.NextX
          end
        end
        if Rl >= Game.Maps.MinY and Rl <= Game.Maps.MaxY and Rx >= Game.Maps.MinX and Rx <= Game.Maps.MaxX then
          if not Game.Maps.Region[Rl][Rx].Grid[l][MapX].Entity or not Game.Maps.Region[Rl][Rx].Grid[l][MapX].Entity.Collide then
            self.Y = self.NextY
          end
        end
